...
public class RenderConfigWindow
{
    ...
    public void Show(RenderConfig config, RenderConfigDefault configDefault, PathPreferences prefs, ref bool paused)
    {
        ...
        if (ImGui.TreeNode("Gamemode Config##config"))
        {
            ...
            using (ImGuiExt.DisabledIf(!config.ShowPickedPlatform))
            ...
            if (ImGui.TreeNode("Horde##config"))
            {
                ...
                using (ImGuiExt.DisabledIf(!config.ShowHordeDoors))
                {
                ...
                using (ImGuiExt.DisabledIf(config.HordePathType == RenderConfig.PathConfigEnum.NONE))
                {
                    config.HordePathIndex = ImGuiExt.SliderInt("Horde path index##config", config.HordePathIndex, 0, 19);
                    using (ImGuiExt.DisabledIf(config.HordePathType != RenderConfig.PathConfigEnum.CUSTOM))
